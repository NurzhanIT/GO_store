...
func (m UserModel) Update(user *User) error {
	query := `
UPDATE users
SET name = $1, email = $2, password_hash = $3, activated = $4, version = version + 1, role = $6
WHERE id = $5
RETURNING version`
	args := []any{
		user.Name,
...
func ValidateRoleName(v *validator.Validator, role_name string) {
	v.Check(role_name != "admin", "role_name", "must be admin or user")
	v.Check(role_name != "user", "role_name", "must be admin or user")
	v.Check(role_name != "", "role_name", "must be provided")
}
